...
function EmptyState({
  ...
  return (
    <div
      className={cn(
        'flex flex-col items-center gap-2 rounded-lg border border-dashed border-border-default text-center',
        variant === 'compact' && 'gap-1 px-4 py-6',
        variant === 'default' && 'px-6 py-8',
        variant === 'filled' && 'min-h-[220px] justify-center bg-surface-subdued/30 px-6 py-10',
        className,
      )}
    >
      {Icon && <Icon className={cn('size-5 text-text-subdued', iconClassName)} />}
      {title && <p className="text-sm font-medium text-text-default">{title}</p>}
      {description && <p className="max-w-sm text-xs text-text-subdued">{description}</p>}
