...
import type { EventFormState, RelatedContentItem } from '@/types/event'
...
function EventDatasetsSection({ form, onChange }: EventDatasetsSectionProps) {
  ...
  const connectedDatasets = form.relatedContent.datasets
  const connectedIds = connectedDatasets.map((d) => d.id)

  const addItem = (item: RelatedContentItem) => {
    onChange((prev) => ({
      ...prev,
      relatedContent: { ...prev.relatedContent, datasets: [...prev.relatedContent.datasets, item] },
    }))
  }
  ...
  const removeItem = (id: string) => {
    onChange((prev) => ({
      ...
      relatedContent: {
        ...prev.relatedContent,
        datasets: prev.relatedContent.datasets.filter((d) => d.id !== id),
      },
    }))
  }

  const handleConnect = (dataset: { id: string; title: string }) => {
    if (replacingId) {
      removeItem(replacingId)
      setReplacingId(null)
    }
    addItem({ id: dataset.id, title: dataset.title, type: 'dataset' })
    setJustAddedMessage(`"${dataset.title}" connected to this event.`)
  }
  ...
  const handleDatasetCreated = (datasetId: string, name: string) => {
    ...
    setCreateOpen(false)
    setJustAddedMessage(`"${name}" created and connected to this event.`)
  }
    ...
    <Card>
      <CardHeader>
        ...
        <p className="mt-1 text-sm font-normal text-muted-foreground">
          Connect datasets that support this contribution.
        </p>
      </CardHeader>
        ...
        {connectedDatasets.length === 0 ? (
          <div className="flex flex-col items-center gap-1 rounded-lg border border-dashed border-border py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Connect an existing published dataset to support this contribution.
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {connectedDatasets.map((item) => {
              const liveDataset = datasets.find((d) => d.id === item.id)
